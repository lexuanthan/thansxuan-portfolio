...
Vì lý do bảo mật, tôi không được phép ghi thẳng file `.env.local` vào máy anh.
Trong thư mục project đã có sẵn file **`ENV-LOCAL.txt`** — chỉ cần **đổi tên nó thành `.env.local`**
(hoặc copy nội dung sang một file mới tên `.env.local`).

Trên Windows, mở PowerShell trong thư mục project và chạy:
