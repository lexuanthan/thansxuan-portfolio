...
**Quy mô:** 9 người · **Nền tảng:** Next.js + Supabase, gắn vào web thanlx.vercel.app
...
```
┌─────────────────────────────────────────────┐
│  Next.js (App Router) — thanlx.vercel.app   │
│  /du-an/cong-viec  ← tool nằm ở đây         │
└──────────────┬──────────────────────────────┘
               │
    ┌──────────┴──────────┐
    │                     │
...
**Tái sử dụng sẵn có từ web hiện tại:** bộ màu hổ phách, `Card`/`Button`/
`Container` trong `components/ui`, cấu hình Supabase, font Be Vietnam Pro.
Không dựng lại design system mới.

**Vị trí:** tool đặt tại `/du-an/cong-viec`. Trang `/du-an` liệt kê từ bảng
`projects` trong database nên phải thêm một dòng vào bảng đó thì thẻ mới hiện
ra — tạo route thôi là chưa đủ.
...
**Bối cảnh.** Tôi có một website cá nhân Next.js 16 (App Router, TypeScript
strict) + Supabase + Tailwind CSS v4, đang chạy tại thanlx.vercel.app. Bộ màu
chủ đạo là hổ phách (`brand-*`), nền kem ấm, font Be Vietnam Pro, các thành phần
dùng chung nằm ở `src/components/ui`. Tôi cần thêm một tool quản lý công việc cho
Ban Thường vụ Đoàn trường gồm **9 người**, đặt tại route `/du-an/cong-viec`.

**Hãy dùng lại bộ thành phần và bộ màu sẵn có, không dựng design system mới.**
...
- Nhớ rằng trang `/du-an` liệt kê từ bảng `projects` trong database, nên phải
  kèm câu SQL thêm một dòng vào bảng đó thì thẻ mới hiện ra.

**Thứ tự thực hiện, làm từng mốc một, xong mốc nào chạy test mốc đó:**

```
M1  Migration + RLS + đăng nhập Google + allowlist + quản lý thành viên
...
2. **Vercel đang dùng gói gì?** Nếu đã có Pro thì cron chạy được mỗi phút và
   kiến trúc đơn giản hơn nhiều — bỏ được `pg_cron`.
